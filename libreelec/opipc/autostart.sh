...
[ -r /storage/.config/kodi.conf ] && . /storage/.config/kodi.conf

cp /usr/share/alsa/alsa.${alsa_output}.conf.min /dev/shm/alsa.conf

if   [ "$alsa_output" = hdmi ]; then
 for i in $(ls /dev/snd/); do
  [ ! "$i" = "pcmC0D0p"  ] && [ ! "$i" = "controlC0"  ] && rm -rf "/dev/snd/$i"
 done
elif [ "$alsa_output" = int  ]; then
 for i in $(ls /dev/snd/); do
  [ ! "$i" = "pcmC1D0p"  ] && [ ! "$i" = "controlC1"  ] && rm -rf "/dev/snd/$i"
 done
else
 for i in $(ls /dev/snd/); do
  [ ! "$i" = "pcmC2D0p"  ] && [ ! "$i" = "controlC2"  ] && rm -rf "/dev/snd/$i"
 done
fi
